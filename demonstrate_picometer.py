positions=[7119, 6500, 6000, 6500, 7119]
import sys
import os
import inspect
...
def adjust_focus():
    print(f"Moving through focus positions: {positions}")
    ...
    if (bStatus) :
        ...
        if (nDeviceCount == 0) :
            ...
            for oDeviceKey in strDeviceKeyList :
                ...
                # Move through each listed target position in order.
                for target in positions:
                    target_steps = int(round(target))
                    cmd = f"4PA{target_steps}"
