"""
keysight_ktxsan Python API Status SRQ Example Program
...
def main():
    ...
    try:
        ...
        driver.status.standard_event.enable_register = keysight_ktxsan.StatusStandardEventFlags.OPERATION_COMPLETE
        driver.status.service_request_enable_register = keysight_ktxsan.StatusByteFlags.STANDARD_EVENT_SUMMARY
        ...
        # Polling loop
        srq_occurred = False
        status_byte = keysight_ktxsan.StatusByteFlags.NONE
        i=0
        while i < 10: # Wait up to 500ms (iterations * sleep time) for SRQ
            ...
            if driver.driver_operation.simulate:
                status_byte = keysight_ktxsan.StatusByteFlags.REQUEST_SERVICE_SUMMARY # Simulate SRQ

            # StatusByteFlags enums have binary weighted integer values so they can be used for bitwise operations.
            if (status_byte.value & keysight_ktxsan.StatusByteFlags.REQUEST_SERVICE_SUMMARY.value) == keysight_ktxsan.StatusByteFlags.REQUEST_SERVICE_SUMMARY.value:
                print("  SRQ detected")
                # Service SRQ
